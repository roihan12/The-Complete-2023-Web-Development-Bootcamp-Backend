...
app.get("/filter", async (req, res) => {
  ...
  console.log(type);
  const result = filterJoke(jokes, type);
  res.status(200).json({
    status: true,
    data: result,
  });
...
app.post("/jokes", async (req, res) => {
  ...
  jokes.push(newJoke);
  const result = getJoke(jokes, newJoke.id);
  res.status(200).json({
    status: true,
    data: result,
  });
...
app.put("/jokes/:id", async (req, res) => {
  let id = parseInt(req.params["id"]);
  const result = getJoke(jokes, id);
  result.jokeText = req.body.text;
  result.jokeType = req.body.type;
...
app.patch("/jokes/:id", async (req, res) => {
  ...
  if (typeof req.body.text === "string" && typeof req.body.type === "string") {
    result.jokeText = req.body.text;
    result.jokeType = req.body.type;
...
app.delete("/jokes/:id", async (req, res) => {
  let id = parseInt(req.params["id"]);
  const result = DeleteJoke(jokes, id);
...
function getRandomJoke(jokes) {
  ...
  // get random item
  const item = jokes[randomIndex];

  return item;
}

function getJoke(jokes, id) {
  const joke = jokes.find((item) => item.id === id);
  return joke;
}

function filterJoke(jokes, type) {
  const filterjokes = jokes.filter((item) => item.jokeType === type);
  return filterjokes;
}

function DeleteJoke(jokes, id) {
